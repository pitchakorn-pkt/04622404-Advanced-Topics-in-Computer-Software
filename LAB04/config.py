...
#
#   ปิด rerank              เปิด rerank
#   1 : 0      MRR 0.8961   0.9482   <- ไม่ใช้ BM25 เลย
#   1 : 0.5    MRR 0.7219   0.9482   <- ค่าที่ใช้
#   1 : 1      MRR 0.6967   0.9496   <- ค่าดั้งเดิมของสูตร RRF
#   1 : 1.5    MRR 0.6165   0.7583
#   0 : 1      MRR 0.4953   0.7583
#
# เมื่อเปิด rerank ค่าในช่วง 0 ถึง 1 ให้ผลเหมือนกันทุกข้อ (เทียบทีละคำถามแล้วต่างกัน 0 ข้อ
# ผิดที่อันดับหนึ่ง 4 ข้อเท่ากันหมด) เพราะตัวจัดอันดับใหม่เรียงผู้เข้ารอบใหม่ทั้งชุดอยู่แล้ว
# ค่านี้จึงมีผลจริงเฉพาะตอนปิด rerank ซึ่งเป็นเหตุผลที่ยังตั้งไว้ที่ 0.5 ไม่ใช่ 0
#
