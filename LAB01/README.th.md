...
ปัญหาหกข้อที่พบใน pipeline นี้เขียนไว้ใน [`problems/`](problems/) —
เกิดอะไรขึ้น เพราะอะไร ตรวจสอบยังไง และการแก้ที่ทำไปแล้วหรือเหตุผลที่ยังไม่ได้แก้
แต่ละข้อมีสคริปต์ที่ทำซ้ำได้จากผลลัพธ์ของแต่ละขั้นที่ commit ไว้

```bash
cd problems && python main.py 0     # ทั้งหกข้อ
```

สี่ในหกข้อไม่มี error ให้เห็นเลย ซึ่งเป็นสิ่งที่ทำให้ pipeline ที่ทุกขั้นถอยกลับอย่างนุ่มนวลได้
กลายเป็นของที่ไล่หาสาเหตุยาก
