# CONTRACT.md — ข้อตกลง API ระหว่าง service ของ **ช่วยด้วย (ChuayDuay)** · v1.7
...
**เส้น `university_rag` ห้ามจบด้วยการบอกผู้ใช้ว่าไม่มีข้อมูล** มีสองจังหวะที่ต้องถอยไป `general_ai`:
1. retrieval คืน `chunks` ว่าง หรือ 05 ล่ม
2. 05 คืน chunk มาแล้ว แต่ generation เรียบเรียงไม่ได้ — สังเกตจาก **`sources` ว่าง และในคำตอบมีประโยค "ไม่พบข้อมูลที่เพียงพอ"** (อยู่ตรงไหนของคำตอบก็ได้ อย่าผูกกับตำแหน่งต้นประโยค เพราะ prompt ของ 06 ให้พูดประโยคนี้ท้ายคำตอบได้)

ทั้งสองกรณีต่อท้ายคำตอบว่า "คำตอบนี้มาจากความรู้ทั่วไป ไม่ได้อ้างอิงเอกสารในคลังความรู้" ยกเว้นตอนที่ตัวสำรองล่มไปด้วยจนเหลือแต่ข้อความว่าระบบไม่ว่าง
...
4. **`/feedback` มาถึงก่อน `/log` ได้** (ผู้ใช้กดให้คะแนนเร็วกว่า log ลง DB) — ห้ามใช้ FK แข็งที่ทำให้ insert ล้ม
...
## Changelog
- **v1.7 (20 ก.ย. 2026)** — §3 เขียนกฎ fallback ของเส้น `university_rag` ให้ครบสองจังหวะ (เดิมเขียนแค่เคส `chunks` ว่าง) หลังเจอจากการยิงจริงว่าคำถามนอกคลังจบด้วยข้อความ "ไม่พบข้อมูล" · แก้เลขเวอร์ชันบนหัวไฟล์ที่ค้างอยู่ที่ v1.4 · ไม่มี field ไหนเปลี่ยน
